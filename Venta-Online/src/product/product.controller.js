...
export const productMasVendido = async (req, res) => {
    try {
        let listaMasVendida = await Product.find({ sold: { $gt: 1 } }) //$gt mayor que
...
export const searchWord = async(req, res) => {
    try{
        ...
        let product = await Product.find({name: {$regex: new RegExp(search, 'i')}}). populate('category',['categoryName', 'description'])
        return res.send(product)
    }catch(err){
        console.error(err)
        return res.status(500).send({msg: 'Error when searching for the product by letter'})
    }
}